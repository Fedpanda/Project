"""
baseline_rf_vx_llm.py

Random Forest benchmarks for VX volatility spikes after posts, using LLM-derived features.
...
def make_pipeline(num_features: List[str], cat_features: List[str]) -> Pipeline:
    ...
    # Conservative RF (weak-signal + rare-event setting)
...
def eval_horizon_spec(
    ...
    """
    Evaluate RF for one horizon + spec.
    ...
    # 2) Build labels for rows where absret observed (still spec-invariant)
    y_train_s = (train[abs_col] > thr).where(train[abs_col].notna())
    y_test_s = (test[abs_col] > thr).where(test[abs_col].notna())

    # 3) Now apply spec feature availability + align y by index
...
def _check_required_columns(df: pd.DataFrame) -> None:
    ...
    if missing:
        raise ValueError(f"dataset_vx_llm missing columns needed for baseline_rf_vx_llm: {sorted(missing)}")
...
if __name__ == "__main__":
    ...
    print("\n=== Random Forest (VX spike targets, LLM features) ===")
    ...
    out_path = out_dir / "baseline_rf_vx_llm.parquet"
